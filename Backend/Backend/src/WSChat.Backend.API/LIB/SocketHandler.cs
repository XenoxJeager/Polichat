...
public abstract class SocketHandler
{
    ...
    public virtual async Task OnConnected(WebSocket socket, Rooms r)
    {
        await Task.Run(() => { Connections.AddSocket(socket, r); });
    }
    ...
    public async Task Send(WebSocket socket, string message)
    {
        ...
        await socket.SendAsync(new ArraySegment<byte>(data, 0, data.Length), WebSocketMessageType.Text, true,
    ...
    {
        var sender = Connections.GetChat(s);
